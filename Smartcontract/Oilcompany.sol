...
function getCompany(uint _index) public view returns (uint, uint, uint, uint, uint, uint, uint, uint, address, address, string memory) {
    company memory c = oil[_index];
    
   return (c.barrel, c.price, c.paymentAmount, c.paymentDate, c.percentage, c.amount, c.timestamp, c.totalShares, c.to, c.chairman, c.paymentMethod);
}
